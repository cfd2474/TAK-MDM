...
def parse_form(policy_type: str, form: _MultiDict) -> dict[str, Any]:
    ...
    for field in form_fields(policy_type):
        ...
        if field.control == "bool":
            ...
            # ⚠️ Paired by index, which is safe for the same reason the activity
            # is: a select always submits something (W196).
        ...
        elif field.control == "kiosk_app_version":
            # ⚠️ One field, one build. The select carries `pin:<sha>` like every
            # other build select; anything else — the empty first option, or a
            # value from before this field existed — leaves the key out, so the
            # policy reads as "no build chosen" rather than as a bad one.
            chosen = (form.get(name) or "").strip()
            if chosen.startswith("pin:"):
                sha = chosen[4:].strip().lower()
                if len(sha) == 64:
                    spec[name] = sha
        ...
        elif field.control in ("package_list", "path_list"):
            ...
            # installs nothing.
            choices = form.getlist(f"{name}__version_choice")
            # ⚠️ **A set of package names, not a list paired by position**
            # (W192). An unchecked checkbox does not submit at all, so index
            # pairing would shift every tick after the first unticked row onto
            # the wrong app — the trap `_kiosk_apps` records, and the cost here
            # is uninstalling an app nobody marked. The control carries its own
            # package name, so there is nothing to line up.
            # The blank filter is tidiness, not a control, and a mutation
            # sweep says so: an unfilled row submits "", which matches no
            # package name because a blank package is skipped above. Kept for
            # the set it produces being the set it claims to be.
            removable = {
                value.strip()
                for value in form.getlist(f"{name}__remove_when_no_longer_required")
            ...
            for i, package in enumerate(packages):
                ...
                if choice.startswith("pin:"):
                    ...
                    if len(sha) == 64:
                        row["artifact_sha256"] = sha
                # Written only when true, so a policy nobody has ticked stays
                # byte-identical to one saved before this field existed.
                if package in removable:
                    row["remove_when_no_longer_required"] = True
                rows.append(row)
            if rows:
                spec[name] = rows
