...
def set_device_profile(session: Session, device_profile: str) -> GooglePlayLink:
    """Change the stored profile without relinking (W251).

    ⚠️ **The token is untouched, and that is the point.** The profile was only
    ever settable at link time, so changing it meant unlink and relink — and a
    relink burns a fresh one-time OAuth token for a change the AAS token does
    not care about. apkeep re-registers a fresh GSF id with Google on every
    call, so the profile is the whole of the "device" and can move freely.

    Accepts any non-empty value, for the same reason `link_account` does: the
    dropdown is a convenience, and a list that drifts from the apkeep in the
    image must not cost an operator the ability to choose.
    """
    link = get(session)
    if link.status is not GooglePlayLinkStatus.LINKED:
        raise GooglePlayLinkError("no Google account is linked, so there is no profile to change")
    chosen = (device_profile or "").strip()
    if not chosen:
        raise GooglePlayLinkError("choose a device profile")
    link.device_profile = chosen
    session.flush()
    logger.info("google play device profile is now %s", chosen)
    return link
