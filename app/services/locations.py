...
#: ⚠️ **No `{s}` subdomain.** `a/b/c.tile.openstreetmap.org` is how tile URLs
#: were written for years and is not what the policy supports any more: *"the
#: sole supported endpoint is https://tile.openstreetmap.org/{z}/{x}/{y}.png —
#: other subdomains or hostnames may be slower or withdrawn without notice."*
#: Leaflet fills `{s}` from its own default `abc`, so the old form sent every
#: request somewhere unsupported.
