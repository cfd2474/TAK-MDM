...
def listed_count(session: Session) -> int:
    """How many policies the policy list shows: live operator profiles plus
    live standalone device policies.

    ⚠️ **This is the only definition of "number of policies" a page may use**
    (W248). The fleet page used to count `policy` rows with no `archived_at`,
    which is a different thing: a profile is *one* policy to an operator but
    one row per section here, and the reserved breach profile is rows too
    while the list hides it. So a deployment with no policy an operator had
    made read "3 policies" the moment Admin → Breach mode was opened, and
    every profile created afterwards raised the number by its section count.
    The list's own empty-state condition is `not profiles and not
    device_policies`; this counts exactly those two collections.
    """
    from app.services import profiles as profile_service

    return len(profile_service.list_profiles(session)) + len(list_tab(session, "device"))
