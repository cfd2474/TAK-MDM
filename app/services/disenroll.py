...
def complete(session: Session, device: Device) -> str:
    ...
    # ⚠️ Recorded **before** the row is deleted, and it survives the deletion
    # because `AlertOccurrence.device_id` is SET NULL rather than CASCADE
    # (W195). The serial goes into the detail for the same reason: in a moment
    # there will be nothing left to look it up from.
    alerts.note_event(session, "device_disenrolled", device, serial)
