...
def newest_base_sha(session: Session, package_name: str) -> str | None:
    """The base APK of this package's newest build, or None if there is none.

    ⚠️ **This is the automatic selection W139 removed, and it exists for
    exactly one caller: the ATLAS launcher** (W196). Read
    `resolve_for_policy` above for why nothing is chosen on an operator's
    behalf any more — a policy naming an app whose newest library build was
    something nobody intended nearly installed Chrome across a fleet.

    The launcher is not that. `dist/atlas-launcher.apk` is shipped by ATLAS and
    loaded by `seed-packages`, so there is no "which build did they mean"
    question to get wrong, and the alternative — asking an operator to pick a
    build for ATLAS's own launcher — is a step they cannot reason about and will
    forget. Forgetting it means a multi-app kiosk that silently does not
    engage, which is the bug this function was written to fix.

    ⚠️ Do not reach for this for an operator's app. If a second caller ever
    seems to want it, the thing being asked for is W139 back again.

    ⚠️ The `BASE` filter is an **equivalent mutant today** and is kept
    anyway: `ingest` happens to write the base part first, so "the first file"
    and "the base" are the same answer and a sweep cannot tell them apart.
    Nothing declares that ordering. A split is not something `PackageInstaller`
    can start from, so if the order ever changes the kiosk fails again one layer
    down, with a less legible error than the one this function was written to
    fix.
    """
    ...
    if version is None:
        return None
    return next(
        (f.artifact_sha256 for f in version.files),
        None,
    )
