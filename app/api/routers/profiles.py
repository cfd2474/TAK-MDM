...
def set_profile_targets(
    ...
    """
    profile = _get(session, profile_id)
    # ⚠️ Hiding the breach profile from the pickers is not a control on its
    # own — a hidden option is still one POST away for anyone with the id, and
    # this profile applied to a group strips every policy from every device in
    # it (W193).
    try:
