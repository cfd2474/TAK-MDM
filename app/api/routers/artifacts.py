...
def download_artifact(
    ...
    if not range_header:
        # ⚠️ `read_all`, not the raw handle. This is the path every managed
        # device uses to install every app, so the line-by-line streaming it
        # used to do was the whole fleet's download speed.
        if not storage.exists(digest):
            raise HTTPException(status.HTTP_404_NOT_FOUND, "unknown artifact")
        return StreamingResponse(
            storage.read_all(digest),
