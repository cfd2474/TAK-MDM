...
def _record_oem_license(device: Device, report: OemLicenseReport | None) -> None:
    """Store what the device said about its OEM licence (W227).

    ⚠️ **Silence leaves the record alone.** An agent too old to report this
    sends nothing, and blanking the columns for it would make a tablet that has
    a perfectly good Knox licence read as having none — sending an operator to
    buy a key they already own. Same rule as the IMEI and the ATAK version.

    ⚠️ **A report, once present, replaces all five columns including with
    nulls.** Unlike the guard above, the fields *within* a report are not
    None-means-unsaid: the device computed one coherent state, and carrying the
    error code of a failure forward onto a licence that has since gone active
    would show an operator a code alongside a working licence.
    """
    if report is None:
        return

    device.oem_license_status = report.status
    device.oem_license_masked_key = report.masked_key
    device.oem_license_error_code = report.error_code
    # `or None` so an agent sending "" stores NULL rather than an empty string —
    # the page renders "no detail" for one and an empty paragraph for the other.
    device.oem_license_detail = report.detail or None
    device.oem_license_activated_at = _from_epoch_millis(report.activated_at)


def _from_epoch_millis(millis: int | None) -> datetime | None:
    """The device reports an instant in epoch milliseconds; we store a datetime.

    ⚠️ **Tolerant of a nonsense value**, because a device's clock is one of
    the things a Knox activation failure is *about* (Samsung's error 205 is
    exactly that). A year-900000 timestamp from a tablet whose clock is wrong
    would otherwise raise out of the check-in handler, and the device would stop
    reporting anything at all — including the licence error that explains it.
    """
    if millis is None:
        return None
    try:
...
def checkin(
    ...
    _record_oem_license(device, payload.oem_license)
