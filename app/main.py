...
def _start_deployment_sweeper(app: FastAPI) -> threading.Thread | None:
    """Put scheduled policies into force at the moment they are due (W191).

    ⚠️ **This thread is a promptness dial, not the mechanism.** It is the third
    of three layers and the only one that can fail without making anything
    wrong:

    1. The resolver reads the clock, so any recompute after the moment is
       correct whether or not this ever ran.
    2. The cache carries the moment as an expiry, so a device's next read
       recomputes — and a parked long poll releases within one slice.
    3. This wakes them instead, so the deployment lands *at* the stated minute.

    That ordering is deliberate and it is the reason the feature does not rest
    on a thread. A background job that flips a flag is the obvious design, and
    it puts one daemon between an operator and a deployment that silently never
    happened — a fleet that did not change looks exactly like a fleet that did
    not need to.

    ⚠️ **This host has no cron**, the same constraint the retention sweep
    documents above, which is why the job lives in the application at all.

    Runs once at boot before the first sleep: a server that was down at 08:00
    activates on the way back up rather than waiting a whole interval, and
    `due` is written so a week of missed schedules lands in one pass.

    ⚠️ Failures are logged and swallowed, and the thread keeps its schedule. A
    sweep that fails is a deployment that waits for a device to ask; a sweep
    that takes the process down is a fleet that stops being managed.
    """
    # ⚠️ Through `dependency_overrides`, for the reason `get_session_factory`
    # gives: a background thread resolving the real factory writes to the real
    # database in the middle of a test run — and this one commits.
    from app.api.deps import get_session_factory

    overrides = app.dependency_overrides
    settings = overrides.get(get_settings, get_settings)()
    interval = settings.deployment_sweep_seconds
    if interval <= 0:
        return None
    ...
    def run() -> None:
        while True:
            sweep_due_deployments(session_factory)
            time.sleep(interval)

    thread = threading.Thread(target=run, name="deployment-sweep", daemon=True)
    thread.start()
    return thread


def _start_alert_sweeper(app: FastAPI) -> threading.Thread | None:
    """Look for the alert conditions that have become true (W195).

    The third thread on the pattern `_start_location_retention` established, and
    for the same reason it gives: **this host has no cron**.

    ⚠️ Five minutes, not one. The shortest threshold an operator may set is
    fifteen minutes, so nothing here is time-critical — and unlike the
    deployment sweep, this pass reads every device against every rule.

    ⚠️ Failures are logged and swallowed, and the thread keeps its schedule.
    A sweep that fails is an alert that waits five minutes; a sweep that raises
    out of the thread is a fleet that stops being watched, silently, which is
    ...
    def run() -> None:
        while True:
            sweep_alert_conditions(session_factory)
            time.sleep(interval)

    thread = threading.Thread(target=run, name="alert-sweep", daemon=True)
    thread.start()
    return thread


def sweep_alert_conditions(session_factory) -> int:
    """One pass: raise what has become true, clear what has stopped. Returns how
    many are newly raised.

    ⚠️ Separate from the thread that schedules it, so what a sweep *does* can
    be tested without waiting on a clock.

    ⚠️ Raising and sending stay separate functions even though one pass does
    both: it is what lets the suppression be tested with no mail server
    anywhere near it, and what lets a failed send be retried without
    re-evaluating the whole fleet.
    """
    from app.services import alerts

    try:
        with session_factory() as session:
            raised = alerts.evaluate(session)
            if raised:
                session.commit()
                log.info("alert sweep: %d newly raised", len(raised))
            # ⚠️ **Sending is a separate step, and runs even when nothing new
            # was raised.** The event hooks record during a *request* — a device
            # enrolling, a breach engaged from the console — so a sweep that only
            # sent what it had just found itself would leave every one of those
            # waiting for an unrelated condition to change. It also means a send
            # that failed last time is retried here.
            sent = alerts.notify(session, get_settings())
            if sent:
                session.commit()
            return len(raised)
    except Exception:
...
def sweep_due_deployments(session_factory) -> int:
    """One pass: activate everything due, and tell the devices. Returns how many.

    Separate from the thread that calls it so that "what a sweep does" can be
    exercised without waiting on a clock — and so the loop above is small enough
    to read in one go.

    ⚠️ Failures are logged and swallowed. A sweep that fails is a deployment
    that waits for a device to ask (layers one and two still hold); a sweep that
    raises out of the thread is a fleet that stops being managed.
    """
    from app.services import deployment

    try:
        with session_factory() as session:
            activated = deployment.sweep(session)
            if activated:
                # ⚠️ The commit is what rings the doorbell. The wake is queued
                # on the session and fires `after_commit`, so a sweep that
                # resolved and did not commit would change nothing and tell
                # nobody — and would do it again a minute later.
                session.commit()
                log.info("deployment sweep: %d scheduled item(s) now live", activated)
            return activated
    except Exception:
        log.warning("deployment sweep failed", exc_info=True)
        return 0
