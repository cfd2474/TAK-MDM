...
class Settings(BaseSettings):
    ...
    #: Membership required to administer. Blank means any authenticated user,
    #: which is only sensible if Authentik already restricts the application.
    #:
    #: ⚠️ **A comma-separated list, and it has to be.** One server can run an
    #: ATLAS per agency, each with its own admin group — and a global
    #: administrator who supports all of them belongs to none of those groups.
    #: A single required group would either lock the global administrator out of
    #: every agency console or leave every console unenforced; both were the
    #: only options before this took a list.
    #:
    #: Any one of the named groups is sufficient. A bare name still works
    #: exactly as it did, which is what every deployed box has.
    ...
    #: Who this deployment is for, shown in the footer of every page.
    #:
    #: One server can run several ATLAS deployments side by side, one per
    #: agency, each on its own hostname — and they look identical once you are
    #: signed in. An administrator who supports more than one needs to know
    #: which console is in front of them before they push a policy or wipe a
    #: tablet, and the hostname is not on screen.
    #:
    #: ⚠️ **Free text, and it reaches a template.** Autoescaping is what keeps
    #: it text; a footer is on every page, so a name containing markup would be
    #: on every page too. `tests/test_web_footer.py` pins that.
    #:
    #: Empty is the ordinary case for a single-agency box and renders nothing —
    #: a footer reading "Agency: " would be worse than no line at all.
    agency_name: str = ""
