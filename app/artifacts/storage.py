...
class ArtifactStorage(ABC):
    ...
    def read_all(self, digest: str) -> Iterator[bytes]:
        """Yield the whole blob in ``_CHUNK`` blocks, for a streaming response.

        ⚠️ **The routers used to hand the open handle straight to
        `StreamingResponse`, and Starlette iterates whatever it is given.**
        Iterating a binary file object yields *lines* — split on ``0x0A`` —
        so a 21 MB APK went out as **230,142 chunks averaging 93 bytes**,
        each one an `iterate_in_threadpool` round trip. Measured: ~90 KB/s
        over loopback, which is four minutes for the agent APK and long
        enough that Android's provisioning gives up mid-download.

        The same file in 1 MiB blocks is 21 chunks. Reading a blob is the
        storage layer's job, and `read_range` below has always done it
        correctly; this is that shape without the range.
        """
