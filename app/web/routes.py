...
from app.services import qr_recipes
...
# The ATLAS console's package (W197), for the kiosk tile the operator cannot
# remove. A global for exactly the reason above: `_kiosk_apps` is an imported
# macro, so a bare name would render as the empty string and the tile would
# submit no package at all.
_TEMPLATES.env.globals["atlas_console_package"] = ATLAS_CONSOLE_PACKAGE
...
# The OEM licence a device reported (W227). Filters rather than context keys, so
# the device page and the fleet list say the same words without either of them
# passing a label through a route.
_TEMPLATES.env.filters["oem_license_label"] = knox_license.status_label
_TEMPLATES.env.filters["oem_license_problem"] = knox_license.is_problem
...
@pass_context
def _deploy_status(ctx: Any, item: Any) -> str:
    """Jinja's `deploy_status(...)`: "live", "held" or "scheduled" (W191).

    ⚠️ A global rather than something each route computes and passes. The
    policy list renders four collections of policies and profiles, and a page
    that forgot one would show a held policy indistinguishable from a live one —
    which is the single worst thing this feature could get wrong in a console.
    """
    return deployment.status(item)


_TEMPLATES.env.globals["deploy_status"] = _deploy_status


@pass_context
def _deploy_input(ctx: Any, item: Any) -> str:
    """Jinja's `deploy_input(...)`: a scheduled moment, for a `datetime-local`.

    ⚠️ Local, not UTC. Re-opening the form has to show the operator the time
    they typed; rendering the stored instant would move the schedule by the zone
    offset every time the form was saved again.
    """
    return clock.to_local_input(
        getattr(item, "effective_at", None), ctx.get("tz") or clock.UTC
    )


_TEMPLATES.env.globals["deploy_input"] = _deploy_input
...
def _render(
    ...
    # Who this deployment serves, for the footer. Injected here for the same
    # reason `build` is: every page shows it, and a page that forgot would
    # render nothing rather than fail, so nobody would notice.
    context["agency_name"] = settings.agency_name.strip()
...
def dashboard(
    ...
    return _render(
        ...
        counts={
            "devices": len(rows),
            # ⚠️ The list's definition, not a row count (W248): a profile is
            # one policy however many sections it stores, and the reserved
            # breach profile is not a policy an operator made.
            "policies": policy_admin.listed_count(session),
...
def _render_group_detail(
    ...
    assignable_profiles = list(
        session.scalars(
            # ⚠️ Reserved profiles out: the breach profile in this dropdown is
            # one click from stripping every policy from every device in the
            # group (W193).
            profile_service.exclude_reserved(
...
@router.post("/devices/{device_id}/breach")
def engage_breach_form(
    device_id: uuid.UUID,
    confirm: str = Form(default=""),
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Put one device into breach mode (W193).

    ⚠️ **Typing the serial is the confirmation**, the same gate disenroll uses
    and for the same reason: this uninstalls the device's apps and empties the
    configured directories, and a dialog anyone can dismiss by reflex is not
    proportionate to that. Naming the device cannot be done by accident on the
    wrong row of a fleet table.

    ⚠️ **Refused when breach mode has never been configured.** A device
    breached against an empty profile loses every policy it had and receives
    nothing — no tracking, no lock note, no passcode — which is strictly worse
    than not breaching at all, and would look like it had worked.
    """
    device = session.get(Device, device_id)
    if device is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "device not found")
    ...
    if confirm.strip() != device.serial_number:
        return _redirect(
            f"/devices/{device_id}?error="
            + _quote("type the serial number exactly to confirm breach mode")
        )

    if breach_service.profile(session) is None:
        return _redirect(
            f"/devices/{device_id}?error="
            + _quote(
                "breach mode has not been set up yet — configure it in "
                "Admin → Breach mode first, so you can see what it will do"
            )
        )

    breach_service.engage(
        session, device, by=None if identity.is_anonymous else identity.username
    )
    session.commit()
    return _redirect(f"/devices/{device_id}?breach=engaged")


@router.post("/devices/{device_id}/breach/clear")
def clear_breach_form(
    device_id: uuid.UUID,
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Take a device back out of breach mode.

    ⚠️ **No serial to type, deliberately.** The gate belongs on the action that
    destroys something. This one restores the device's policies exactly as they
    were — nothing was ever unassigned — and making it hard would mean a
    misdirected breach stayed in force while somebody found the right row.

    It does not undo the damage, and the page says so where it is pressed.
    """
    device = session.get(Device, device_id)
    if device is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "device not found")

    breach_service.clear(session, device)
    session.commit()
    return _redirect(f"/devices/{device_id}?breach=cleared")
...
def _catalog_view(
    ...
    for category in creator_catalog.CATALOG:
        if category.reserved:
            continue  # breach mode is edited in Admin, never in a profile (W193)
        section = (
...
def new_single_policy_page(
    ...
    # ⚠️ Reserved categories excluded. A standalone BREACH policy could be
    # assigned to a group like any other, and would empty those directories on
    # every device in it with no breach engaged (W193).
    wired = creator_catalog.wired_categories()
    return _render(
        request,
        "policy_new.html",
        identity=identity,
...
def create_profile_form(
    ...
    name: str = Form(...),
    description: str = Form(default=""),
    state: str = Form(default=deployment.LIVE),
    effective_at: str = Form(default=""),
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    form = _sync_form(request)
    ...
    message = _hold_if_asked(session, profile, state, effective_at)
    if message:
        session.commit()
        return _redirect(f"/profiles/{profile.id}?deployment={_quote(message)}")
    return _redirect(f"/profiles/{profile.id}")


@router.get("/profiles/{profile_id}", response_class=HTMLResponse, response_model=None)
def profile_detail(
    ...
    if profile_service.is_reserved(profile):
        # ⚠️ Sent to the one page that can edit it, rather than rendered
        # half-usable here. This editor offers "Assign to devices" and an editor
        # for every section — and for the breach profile the assignment is
        # refused and the app blocklist is generated, so most of what it offered
        # would either error on submit or be silently overwritten (W193).
        return _redirect("/admin#tab-breach")
...
def set_profile_targets_form(
    ...
    if profile is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "profile not found")
    try:
        profile_service.refuse_if_reserved(profile)
    except profile_service.ProfileError as exc:
...
def create_policy(
    ...
    # ⚠️ After the commit that creates it, not before: `hold` invalidates and
    # wakes, and a policy that does not exist yet has no assignments to wake
    # anyone over. It has none at this point either — targets are added on the
    # detail page — which is exactly why asking now is free.
    message = _hold_if_asked(session, policy, state, effective_at)
    if message:
        session.commit()
        return _redirect(f"/policies/{policy.id}?deployment={_quote(message)}")
...
def _hold_if_asked(session: Session, item: Any, state: str, effective_at: str) -> str:
    """Apply a deployment choice made while *creating* something.

    Returns a message to append to the redirect, or "" when the choice was the
    default. ⚠️ A bad date does not lose the policy the operator just built:
    it is created live-by-default and the error is reported, because refusing
    the whole creation over a mistyped date would discard a form that may have
    taken ten minutes to fill in.
    """
    if state == deployment.LIVE:
        return ""
    try:
        return deployment.choose(session, item, state, effective_at)
    except ValueError as exc:
        return f"{item.name} was created live: {exc}"


@router.post("/policies/{policy_id}/deployment")
def set_policy_deployment(
    policy_id: uuid.UUID,
    state: str = Form(...),
    effective_at: str = Form(default=""),
    session: Session = Depends(get_db),
    ...
    """Live, held, or scheduled — for one standalone policy (W191)."""
    policy = session.get(Policy, policy_id)
    if policy is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "policy not found")
    return _apply_deployment(session, policy, state, effective_at, f"/policies/{policy_id}")


@router.post("/profiles/{profile_id}/deployment")
def set_profile_deployment(
    profile_id: uuid.UUID,
    state: str = Form(...),
    effective_at: str = Form(default=""),
    session: Session = Depends(get_db),
    ...
    """The same three choices for a whole profile, which is what most operators
    are actually holding back — `/policies/new` builds profiles."""
    profile = profile_service.get_profile(session, profile_id)
    if profile is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "profile not found")
    return _apply_deployment(session, profile, state, effective_at, f"/profiles/{profile_id}")
...
def _apply_deployment(
    ...
    """Shared by both routes, because the two differ only in what they look up.

    ⚠️ **The confirmation is carried back, not dropped.** The operator typed a
    local wall-clock time and the database holds an instant; a console timezone
    set wrongly puts the deployment in the middle of the night, and this echo is
    the only place that difference is ever visible.
    """
    try:
        message = deployment.choose(session, item, state, effective_at)
    except ValueError as exc:
        # ⚠️ Insurance, not a control, and said plainly because a mutation sweep
        # would otherwise report it as dead code. Every path that raises in
        # `choose` does so *before* it writes anything — an unreadable date or an
        # unknown state both fail on the way in — so today there is nothing to
        # undo. It stays because the alternative to an unnecessary rollback is a
        # half-applied deployment redirecting to a page that reports an error,
        # which is the one outcome nobody could debug from the console.
        session.rollback()
        return _redirect(f"{back_to}?error={_quote(str(exc))}")
    session.commit()
    return _redirect(f"{back_to}?deployment={_quote(message)}")
...
def enrollment_page(
    ...
    return _render(
        ...
        groups=list(session.scalars(select(DeviceGroup).order_by(DeviceGroup.name))),
        # ⚠️ A shelf of **live credentials**, not a history of past actions
        # (W204). `live` drops anything whose token has been retired, expired or
        # used up, because a row that enrols nothing sitting beside rows that do
        # is worse than no list at all.
        saved_qrs=_saved_qr_rows(session),
...
def _saved_qr_rows(session: Session) -> list[dict[str, Any]]:
    """The permanent QRs this console can draw again, for the Enroll page."""
    rows: list[dict[str, Any]] = []
    for recipe in qr_recipes.live(session):
        group = (
        ...
        rows.append(
            {
                "id": recipe.id,
                "label": qr_recipes.describe(recipe, group),
                "wifi_ssid": recipe.wifi_ssid,
                "created_at": recipe.created_at,
                "last_shown_at": recipe.last_shown_at,
            }
        )
    return rows


@router.get("/enrollment/qr/{recipe_id}/show")
def show_saved_qr_get(recipe_id: uuid.UUID) -> RedirectResponse:
    """A GET here means something turned the POST back into a navigation.

    ⚠️ **A redirect, never the QR.** The POST renders a standing credential,
    so its URL is in the address bar afterwards — a reload, a bookmark, a
    prefetch, or an expired session bounced through the identity provider all
    come back as a GET. Drawing the code for any of those would put a live
    enrollment credential on screen for a navigation nobody made on purpose.

    Answering at all is the point: without this the replay is
    `{"detail": "Method Not Allowed"}` as JSON, which is not a sentence anyone
    can act on — observed live on the sibling route.
    """
    return _redirect("/enrollment")


@router.post(
    "/enrollment/qr/{recipe_id}/show",
...
def show_saved_qr(
    ...
    """Draw a permanent QR again, from what it was made with (W204, issue #2).

    ⚠️ **This mints nothing.** A permanent QR carries the token's own secret,
    so rendering the same options against the same token reproduces the same
    picture — which is the entire reason recall is possible without keeping a
    copy of the code anywhere.

    ⚠️ **POST, not GET.** The result is a standing credential on screen; a GET
    would put it in browser history, in a bookmark, and in any link an operator
    pasted to a colleague.
    """
    recipe = qr_recipes.get(session, recipe_id)
    if recipe is None:
        return _redirect("/enrollment?error=that+saved+QR+is+no+longer+here")

    if recipe not in qr_recipes.live(session):
        # Its token was retired, expired or used up. Drawing the picture would
        # hand someone a code that cannot enrol anything, and they would only
        # find out after a factory reset.
        return _redirect(
            "/enrollment?error=that+QR%27s+token+has+been+retired%2C+so+it+would+"
            "no+longer+enrol+anything"
        )

    options = qr_recipes.options(session, recipe, vault)
    if not options.wifi_recoverable:
        # ⚠️ Refused rather than drawn without the network. A QR missing its
        # Wi-Fi scans, looks right, and leaves the tablet unable to reach
        # anything — a failure that surfaces far from its cause.
        return _redirect(
            "/enrollment?error=the+Wi-Fi+password+for+that+QR+cannot+be+"
            "recovered+%28the+vault+key+changed%29.+Generate+it+again+to+re-save+it"
        )

    qr_recipes.mark_shown(session, recipe)
    session.commit()
    ...
    return _render_primary_qr(
        ...
        wifi_security=options.wifi_security or "WPA",
        group=options.group,
        vault=vault,
        persistent=True,
    )
...
@router.post("/enrollment/qr/{recipe_id}/forget")
def forget_saved_qr(
    recipe_id: uuid.UUID,
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Drop a saved QR from the shelf.

    ⚠️ **This does not revoke anything.** The QR it described goes on working,
    because it carries the token's own secret — only "Retire & create new" stops
    that. The page says so next to the button, because an operator who believed
    otherwise would think they had cancelled a credential they had not.
    """
    recipe = qr_recipes.get(session, recipe_id)
    if recipe is not None:
        qr_recipes.forget(session, recipe)
        session.commit()
    return _redirect("/enrollment?forgotten=1")
...
def create_primary(
    ...
    """
    # ⚠️ **No group scoping here** (W207). Which group a device lands in is a
    # property of the QR it was enrolled with, not of the token: picking a group
    # on the QR form resolves to that group's own standing token (W121). Reading
    # `group_ids` off this form as well meant one decision could be made in two
    # places, and a primary scoped to one group with a QR scoped to another had
    # an answer nobody could guess from the page.
    #
    # Dropped rather than ignored: a direct POST carrying `group_ids` would
    # otherwise still scope the primary, and the console would show no sign of it.
    retire_and_create_primary(
        session,
        name=name,
        created_by=None if identity.is_anonymous else identity.username,
...
def generate_qr_get() -> RedirectResponse:
    ...
    ⚠️ A POST that renders a page leaves its URL in the address bar, and
    anything replaying it arrives as a GET. (W204 added a second such route,
    `/enrollment/qr/{recipe_id}/show`, which answers a GET the same way and for
    the same reason.) Observed live: an Authentik session lapsed mid-form, the edge sent the
...
def _render_primary_qr(
    ...
    *,
    ...
    if persistent:
        # ⚠️ **After the payload is built, not before** (W204). Everything above
        # can still refuse — an unrecoverable secret, a provisioning error — and
        # a shelf entry for a QR that was never drawn would offer a picture
        # nobody has ever seen.
        #
        # Only persistent QRs. A fifteen-minute derivative is a different
        # credential each time, so remembering one would be a list of pictures
        # that stopped working before anybody clicked them.
        qr_recipes.remember(
            session,
            primary,
            group=group,
            ...
            wifi_password=wifi_password,
            vault=vault,
            created_by=getattr(identity, "subject", None),
        )
        session.commit()
...
def download_app_version(
    ...
    if len(parts) == 1:
        part = parts[0]
        if not storage.exists(part.artifact_sha256):
            raise HTTPException(
                status.HTTP_404_NOT_FOUND, "the stored file is missing"
            )
        # ⚠️ `read_all`, not the raw handle: see `ArtifactStorage.read_all`.
        return StreamingResponse(
            storage.read_all(part.artifact_sha256),
...
def apps_page(
    ...
    return _render(
        ...
        identity=identity,
        packages=packages,
        # ⚠️ Measured on `artifact_dir`, which is `/artifacts` in the
        # container — the filesystem the uploads actually live on. Where the
        # InfraTAK module has reserved a store (W205) that is the reservation,
        # so the meter counts against the provision rather than the box's disk.
        storage_usage=storage_usage.survey(session, settings.artifact_dir),
        storefronts=storefront_service.list_all(session),
...
def _repo_source(
    name: str, settings: Settings, session=None, vault=None, *, device_profile: str = ""
):
    """The source by name, built against the on-disk cache.

    `session` and `vault` are needed only by Google Play, which cannot exist
    without a linked account; every other source ignores them. `device_profile`
    overrides the link's stored profile for this one request (W251) — safe
    because the Play source is built per request, never cached.
    """
    ...
    if session is not None and vault is not None:
        ...
        chosen = (device_profile or "").strip()
        if play is not None and chosen:
            email, token, _stored = play
            play = (email, token, chosen)
...
def repo_import_form(
    ...
    `device_profile` is honoured for Google Play only (W251): it is the one
    source that serves device-matched builds, and the choice is recorded in the
    build's provenance ("device …") so it is explicable afterwards.
    """
    from app.services import import_jobs, repo_import
    from app.services.app_sources.base import SourceError

    client = _repo_source(source, settings, session, vault, device_profile=device_profile)
    if client is None:
        return JSONResponse({"error": f"unknown source {source!r}"}, status_code=404)
...
def content_raw(
    ...
    if not storage.exists(managed.artifact_sha256):
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, "the stored artifact is missing"
        )
    # ⚠️ `read_all`, not the raw handle: see `ArtifactStorage.read_all`.
    # Found by the guard in `test_artifact_streaming`, not by hand -- this is
    # the fourth site with the same shape and the one that was missed.
    return StreamingResponse(
        storage.read_all(managed.artifact_sha256),
...
def admin_page(
    ...
    return _render(
        ...
        breach=_breach_panel(session),
        alert_recipients=alert_service.list_recipients(session),
        alert_rules_list=alert_service.list_rules(session),
        alert_catalog={
            "events": alert_rules.EVENTS,
            "units": list(alert_rules.UNITS),
            "fields": alert_rules.FIELDS,
            "operators": alert_rules.OPERATORS,
            "describe": alert_rules.describe,
        },
        mail=_mail_panel(session, settings),
        **_form_catalogs(session),
    )


#: The sections Admin → Breach mode edits, in the order they are shown.
#:
#: ⚠️ The app blocklist is **not** here. It is generated from the library and
#: shown read-only — offering it as a form would invite an operator to edit a
#: value that `breach.regenerate` overwrites the next time anyone is breached,
#: which is the worst kind of control: one that appears to work.
_BREACH_SECTIONS = (
    ("breach", "What is deleted"),
    ("password", "The passcode the device is locked with"),
    ("customizations", "What the lock screen says"),
    ("tracking_fencing", "How often it reports where it is"),
    ("wallpaper", "What is shown on screen"),
)


def _breach_panel(session: Session) -> dict:
    """Admin → Breach mode: what a breached device gets, and what it destroys.

    ⚠️ The profile is created **on first view of this tab**, not on first
    breach. An operator has to be able to read and change what the button will
    do *before* pressing it — a page that said "nothing is configured" and a
    button that silently configured itself would be the same feature with the
    review step removed.
    """
    fresh = breach_service.profile(session) is None
    profile = breach_service.ensure(session)
    if fresh:
        # ⚠️ Committed, or the profile this page just created is rolled back
        # when the request's session closes and the next view creates it again —
        # for ever, with the tab reporting nothing configured each time. Only
        # when it is actually new: an unconditional commit here would also
        # persist whatever else a request happened to have pending.
        session.commit()
    specs = {
        key: (
            section.latest_version.spec
            if (section := profile_service.section_for(profile, key))
            and section.latest_version
            else {}
        )
        for key, _ in _BREACH_SECTIONS
    }
    return {
        "profile": profile,
        # Live, so the page is never showing a library that has moved on. The
        # frozen copy is written at engage time and read only by devices.
        "blocklist": breach_service.blocklist(session),
        "sections": [
            {
                "key": key,
                "heading": heading,
                "label": creator_catalog.get(key).label,
                "fields": form_schema.form_fields(creator_catalog.get(key).policy_type),
                "spec": specs[key],
            }
            for key, heading in _BREACH_SECTIONS
        ],
    }


@router.post("/admin/breach/{category_key}")
def save_breach_section_form(
    category_key: str,
    request: Request,
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Publish one section of the reserved breach profile.

    ⚠️ Only the sections this tab owns. `category_key` arrives from the URL, so
    without the check a `POST` to `/admin/breach/app_management` would let
    somebody write the blocklist by hand — and it would appear to work until the
    next engage silently regenerated it.
    """
    allowed = {key for key, _ in _BREACH_SECTIONS}
    if category_key not in allowed:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"{category_key!r} is not a breach setting"
        )
    ...
    except (profile_service.ProfileError, PolicyTypeError, ValueError) as exc:
        session.rollback()
        return _redirect(f"/admin?error={_quote(str(exc))}#tab-breach")
    session.commit()
    return _redirect(f"/admin?saved={_quote(category.label)}#tab-breach")


def _mail_panel(session: Session, settings: Settings) -> dict:
    """What Admin -> Email needs to grey itself out honestly (W194).

    ⚠️ The template cannot work any of this out. It has the stored settings
    and nothing else — not whether the operator is inheriting, not what the
    relay's address is, and certainly not whether the relay exists.
    """
    config = mail.effective_config(session, settings)
    return {
        "inheriting": config.inherited,
        # What the greyed boxes show. Only the values that are genuinely
        # InfraTAK's: a blank one leaves the box grey and empty, which is what
        # the operator asked for and is also the truthful rendering.
        "shown": {
            "smtp.host": config.host,
            "smtp.port": str(config.port) if config.port else "",
            "smtp.from_address": config.from_address,
        },
        # ⚠️ An **inference**, not a check. A blank from-address means the
        # module has never pushed relay settings into this deployment, which is
        # good evidence the relay was never deployed — and is also what a stale
        # `.env` looks like on a box where the relay was set up *after* ATLAS.
        # The definitive answer is whether port 25 answers, which is a socket
        # this page deliberately does not open on every render.
        "looks_unconfigured": not settings.infratak_mail_from.strip(),
    }
...
@router.post("/admin/google-play/device")
def google_play_device_form(
    device_profile: str = Form(default=""),
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Change the stored device profile without relinking (W251)."""
    from app.services import google_play_link

    try:
        google_play_link.set_device_profile(session, device_profile)
        session.commit()
    except google_play_link.GooglePlayLinkError as exc:
        return _redirect(f"/admin?tab=googleplay&play_error={_quote(str(exc))}")
    return _redirect("/admin?tab=googleplay&play_device=1")
...
@router.post("/admin/mail/probe")
def probe_mail_relay_form(
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Open a socket to the configured relay and report what answered (W194).

    ⚠️ **On a button, never on page render.** A page that opens a socket
    every time it loads is a page that hangs whenever the relay host is busy or
    silent — and Admin is the page an operator reaches for when something is
    already wrong.

    ⚠️ The wording of the result matters as much as the result. This proves
    a relay answered and greeted us as a mail server. It does not prove mail
    will arrive, and nothing in ATLAS sends any.
    """
    settings = get_settings()
    result = mail.probe(mail.effective_config(session, settings))
    key = "relay_ok" if result.reachable else "relay_bad"
    return _redirect(f"/admin?{key}={_quote(result.detail)}#tab-smtp")
...
def save_admin_settings_form(
    ...
    form = _sync_form(request)
    fields = settings_store.GROUPS[group_key].fields
    values = {f.key: (form.get(f.key) or "") for f in fields}
    # Checkboxes: present means "on".
    for f in fields:
        if f.kind == "bool":
            values[f.key] = "true" if form.get(f.key) else ""

    # ⚠️ **A `disabled` input submits nothing, and this group greys most of
    # itself out** (W194). While ATLAS is inheriting InfraTAK's relay the six
    # SMTP fields render disabled, so they arrive absent — and the loop above
    # turns absent into `""`, which would quietly erase the operator's own mail
    # server the first time they pressed Save on a page they had not edited.
    #
    # Those settings are precisely what ATLAS falls back to when the toggle goes
    # off again, so absent has to mean "unchanged" here.
    #
    # ⚠️ **Keyed on what was *stored*, not on what was submitted**, and the
    # difference is the whole point. The first version asked whether the
    # incoming payload was inheriting — which is false on the one submit that
    # needs these settings most: unticking the box. The fields are still
    # disabled in that page's DOM, so they arrive absent, and the operator's own
    # server would be wiped by the very click meant to return to it. Found by a
    # test that replayed what a browser actually sends.
    #
    # A field that is *present* and empty is still an operator clearing it, so
    # only absent keys are filled.
    #
    # ⚠️ The `group_key == "smtp"` test is **scoping, not the safety**, and a
    # mutation sweep proves it: removing it changes nothing, because the lookup
    # below is already the smtp group and no other group shares a key with it.
    # What would be dangerous is widening the *lookup* to `group_key` — then
    # "absent means unchanged" would apply everywhere, and an unchecked checkbox
    # is absent in every group, so no boolean setting in the console could ever
    # be turned off again.
    if group_key == "smtp" and mail.inheriting(session):
        stored = settings_store.group_values(session, "smtp")
        for f in fields:
            if f.key != mail.INHERIT_KEY and form.get(f.key) is None:
    ...
    if group_key in ("location", knox_license.SETTINGS_GROUP):
        # Both of these resolve into every device's effective policy, so changing
        # one here changes what the fleet should be doing. Without this the new
        # value would reach only the devices whose policy happened to be
        # recomputed for some other reason — an interval, or a Knox licence, in
        # force on some tablets and not others, with nothing to distinguish them.
        #
        # ⚠️ Invalidation alone is not enough for the Knox key and is not what
        # carries it: `refresh` only bumps `state_version` — which is what makes
        # the server send a bundle at all — because the licence fingerprint is in
        # its comparison. This just makes that recomputation happen now rather
        # than whenever something else disturbed the device.
        eff.invalidate_all(session)
    session.commit()
    return _redirect(f"/admin?saved={group_key}#tab-{group_key}")


@router.post("/admin/alerts/rules")
def add_alert_rule_form(
    request: Request,
    kind: str = Form(...),
    name: str = Form(default=""),
    session: Session = Depends(get_db),
    ...
    """Save one alert rule (W195).

    ⚠️ The config is assembled per kind from the form rather than accepted
    as JSON. A JSON textarea would be quicker to build and would make every
    validation message in `alert_rules` arrive too late to help — the operator
    would be editing a structure instead of answering a question.
    """
    form = _sync_form(request)
    config: dict[str, Any]
    if kind == "event":
        config = {"event": form.get("event")}
    elif kind == "stale":
        config = {"amount": form.get("amount"), "unit": form.get("unit")}
    elif kind == "custom":
        config = {
            "conditions": [
                {"field": field, "op": op, "value": value}
                for field, op, value in zip(
                    form.getlist("field"), form.getlist("op"), form.getlist("value")
                )
                if (field or "").strip()
            ]
        }
        ...
        return _redirect(
            f"/admin?error={_quote(f'{kind!r} is not a kind of alert')}#tab-general"
        )
    ...
    try:
        alert_service.add_rule(
            ...
            config=config,
            name=name,
            by=None if identity.is_anonymous else identity.username,
        )
        session.commit()
    except alert_service.AlertError as exc:
        session.rollback()
        return _redirect(f"/admin?error={_quote(str(exc))}#tab-general")
    return _redirect("/admin#tab-general")


@router.post("/admin/alerts/rules/{rule_id}/enabled")
def set_alert_rule_enabled_form(
    rule_id: uuid.UUID,
    ...
    alert_service.set_rule_enabled(session, rule, enabled.strip().lower() == "true")
    session.commit()
    return _redirect("/admin#tab-general")


@router.post("/admin/alerts/rules/{rule_id}/delete")
def delete_alert_rule_form(
    rule_id: uuid.UUID,
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    ...
    if rule is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "alert not found")
    alert_service.remove_rule(session, rule)
    session.commit()
    return _redirect("/admin#tab-general")


@router.post("/admin/alerts/recipients")
def add_alert_recipient_form(
    address: str = Form(...),
    label: str = Form(default=""),
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Put an address on the alert list (W195)."""
    try:
        alert_service.add(
            session,
            address=address,
...
@router.post("/admin/alerts/recipients/{recipient_id}/enabled")
def set_alert_recipient_enabled_form(
    recipient_id: uuid.UUID,
    enabled: str = Form(default=""),
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    """Turn a recipient on or off.

    ⚠️ Separate from delete, and that is the point of having both. An
    address that is bouncing needs to stop receiving *today*; losing the record
    of who was on the list is a different and worse thing to do about it.
    """
    recipient = session.get(AlertRecipient, recipient_id)
    if recipient is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "recipient not found")
    alert_service.set_enabled(session, recipient, enabled.strip().lower() == "true")
    session.commit()
    return _redirect("/admin#tab-general")


@router.post("/admin/alerts/recipients/{recipient_id}/delete")
def delete_alert_recipient_form(
    recipient_id: uuid.UUID,
    session: Session = Depends(get_db),
    identity: AdminIdentity = Depends(admin_required),
) -> RedirectResponse:
    ...
    if recipient is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "recipient not found")
    alert_service.remove(session, recipient)
    session.commit()
    return _redirect("/admin#tab-general")
