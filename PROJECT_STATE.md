...
1. Replace W115's confirmation dialog with PIN entry.
2. `ApiClient` call to the new endpoint, with the enrollment token.
3. Distinguish the three outcomes in the UI: accepted, refused, could not reach
   the server. Anything but accepted leaves the block in place.
4. Strings and tests.
5. Build, publish, and verify on hardware during the next provisioning run.
